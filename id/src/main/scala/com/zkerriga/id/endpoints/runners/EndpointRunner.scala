...
trait EndpointRunner:
  def run[Service: Tag, E <: EndpointError, A](
    call: Service => IO[E, A]
  ): URIO[Service & ErrorHandler, Either[E | InternalError, A]]
  ...
  object Simple extends EndpointRunner {
    def run[Service: Tag, E <: EndpointError, A](
      ...
      for
        ...
        // todo: can add requestId generation here
        // todo: probably some metrics processing here
        _      <- ZIO.logInfo("Request processing has been started")
        result <- handler.handle(call(service))
        _      <- ZIO.logInfo("Request processing has been finished")
      yield result
  }

  private type Call[Service, E, A] = Service => IO[E, A]

  def on[Service: Tag]: [E <: EndpointError, A] => Call[Service, E, A] => URIO[Service & ErrorHandler & EndpointRunner, Either[E | InternalError, A]] =
    [E1 <: EndpointError, A1] =>
